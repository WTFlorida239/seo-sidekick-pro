...
create policy if not exists "Owners can select own websites"
  on public.websites for select
  using (auth.uid() = owner_user_id);

create policy if not exists "Owners can insert websites"
  on public.websites for insert
  with check (auth.uid() = owner_user_id);

create policy if not exists "Owners can update own websites"
  on public.websites for update
  using (auth.uid() = owner_user_id);

create policy if not exists "Owners can delete own websites"
  on public.websites for delete
  using (auth.uid() = owner_user_id);

-- Audits: accessible only if user owns the parent website
create policy if not exists "Owners can select audits for own websites"
...
create policy if not exists "Owners can insert audits for own websites"
...
create policy if not exists "Owners can update audits for own websites"
...
create policy if not exists "Owners can delete audits for own websites"
